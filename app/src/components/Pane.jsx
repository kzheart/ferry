...
export function Pane({ collapsed, width, dragging, title, count, placeholder,
  ...
  return (
    <div style={{ width: w, flex: "none", overflow: "hidden", background: "#F3F5F7",
      borderRight: collapsed ? "none" : "1px solid #E1E7EC",
      ...
      <div style={{ width, height: "100%", display: "flex", flexDirection: "column",
        ...
        <div style={{ flex: "none", padding: "13px 14px 0" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
            <span style={{ fontSize: 14, fontWeight: 650, color: "#1C2530", letterSpacing: "-.01em" }}>{title}</span>
            <span className="mono" style={{ fontSize: 12, color: "#9AA3AD" }}>{count}</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, height: 30, padding: "0 10px",
            background: "#fff", border: "1px solid #E1E7EC", borderRadius: 7, marginTop: 11 }}>
            <SearchIcon />
            <input placeholder={placeholder} value={query} onChange={onQuery}
              style={{ border: "none", outline: "none", background: "transparent",
                fontSize: 12.5, flex: 1, color: "#1C2530" }} />
          </div>
          <div data-guide="search" style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 8 }}>
            <button onClick={onFilter} style={{ height: 26, display: "flex", alignItems: "center",
              gap: 6, padding: "0 10px", background: filterOn ? "#EAF0FB" : "#fff",
              border: `1px solid ${filterOn ? ACCENT : "#E1E7EC"}`, borderRadius: 7,
              fontSize: 12, color: "#334155", cursor: "pointer" }}>
              <FilterIcon />筛选{filterCount > 0 ? ` · ${filterCount}` : ""}
            </button>
            <span style={{ flex: 1 }} />
            <span style={{ fontSize: 11.5, color: "#8A939D", display: "flex", alignItems: "center", gap: 4 }}>
              {sortLabel}<SortCaret />
            </span>
          </div>
          {tokens.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 9, animation: "fslide .14s ease" }}>
              {tokens.map((tk, i) => (
                <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 5, height: 22,
                  padding: "0 6px 0 9px", background: "#E7F0FE", border: "1px solid #CBDDF8",
                  borderRadius: 20, fontSize: 11, color: "#0B4FBE" }}>
                  {tk.label}
                  <a onClick={tk.onRemove} style={{ color: "#6E8FC4", fontSize: 13, lineHeight: 1 }}>×</a>
                </span>
              ))}
            </div>
          )}
        ...
        <div style={{ flex: "none", borderTop: "1px solid #E4E9EE", padding: "9px 13px",
          fontSize: 11, color: "#9AA3AD", background: "#F0F3F5", whiteSpace: "nowrap",
...
export function PaneEmpty({ text, onClear }) {
  return (
    <div style={{ textAlign: "center", padding: "34px 12px", color: "#9AA3AD" }}>
...
const rowSel = on => ({
  background: on ? "#E7F0FE" : "transparent",
  ...
  return groups.map(g => (
    <div key={g.key} style={{ marginBottom: 3 }}>
      <div className="hov-row" onClick={g.onToggle}
        ...
        <span style={{ fontSize: 11, fontWeight: 600, color: "#5B6672" }}>{g.label}</span>
        <span style={{ fontSize: 11, color: "#9AA3AD" }}>· {g.count}</span>
      </div>
      {g.expanded && (
            ...
            <div key={r.id} onClick={r.onClick} title={r.dir}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12.5, fontWeight: 600, color: "#1C2530", whiteSpace: "nowrap",
                    overflow: "hidden", textOverflow: "ellipsis", flex: 1 }}>{r.title}</span>
                  <span style={{ fontSize: 10.5, color: "#9AA3AD", flex: "none" }}>{r.active}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 2 }}>
                  <span className="mono" style={{ fontSize: 10.5, color: "#8A939D", whiteSpace: "nowrap",
                    overflow: "hidden", textOverflow: "ellipsis" }}>{r.repo}</span>
                  {r.hasSub && <span style={{ fontSize: 10, color: "#6B7682", background: "#EEF2F6",
  ...
  return groups.map(g => (
    <div key={g.label} style={{ marginBottom: 5 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 8px 4px" }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: "#5B6672" }}>{g.label}</span>
        <span style={{ fontSize: 11, color: "#9AA3AD" }}>· {g.rows.length}</span>
      </div>
      {g.rows.map(h => (
        <div key={h.id} onClick={h.onClick}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 10.5, color: "#9AA3AD", flex: "none" }}>{h.short}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
              <span style={{ fontSize: 10.5, color: "#8A939D", whiteSpace: "nowrap", overflow: "hidden",
...
export function SnapList({ rows, empty, onClear }) {
  ...
  return rows.map(s => (
    <div key={s.id} onClick={s.onClick}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span className="mono" style={{ fontSize: 11.5, fontWeight: 600, color: "#334155",
            whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flex: 1 }}>{s.id}</span>
          <span style={{ fontSize: 10.5, color: "#9AA3AD", flex: "none" }}>{s.short}</span>
        </div>
        <div style={{ marginTop: 2, fontSize: 11, color: "#1C2530", whiteSpace: "nowrap",
          overflow: "hidden", textOverflow: "ellipsis" }}>{s.title}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
          <span style={{ fontSize: 10.5, color: "#8A939D" }}>{s.trigger}</span>
