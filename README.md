...
Cursor 可作为迁移来源（把 Cursor 会话迁到其它 Agent），但不再支持迁入 Cursor。
需要在 Cursor 里接着聊时，请用 **续聊**（`ferry-resume`）接手。
...
原生迁移保真但有前提（目标格式可写、目标 Agent 支持迁入）。**续聊**是另一条永远走得通的路：
...
就是换一个干净的上下文接着做。迁移被拒绝时（例如目标不支持迁入），Ferry 也会把这条
指令作为兜底提供。
