...
Everything the CLI prints is the engine's payload passed through verbatim. If a field is
absent or named differently in a real response, trust the JSON you got, not this document.
`ferry help <command> --json` prints the exact option definitions the parser uses.
...
- **All times are UTC.** `--since` / `--until` accept `YYYY-MM-DD`, `YYYY-MM-DDTHH:MM[:SS]`
  (UTC, no local timezone), relative amounts `30s` / `90m` / `24h` / `7d` / `2w`, and
  `@<epoch-ms>` for replaying a resolved boundary. Prefer relative forms. Anything else
  ("yesterday", `2024/01/01`) is an error, never a guess.
- Session references look like `fsr_...` and are issued by the live engine instance. They
  die when the engine restarts or the session file changes; re-run `ferry search` for a
  fresh one. Native ids (`session_id`) are the stable handle for humans and for
  `--session-id`.
- Content search is served by a persistent full-text index. While it is warming, responses
  say so (`content_index.ready: false`) instead of silently returning partial results.
...
- `sessions[]` — `tool`, `ref`, `session_id`, `title`, `title_source`, `project`,
  `updated`, `model`, `revision`, `record_count`, plus `matched_in` (`metadata` / `content`),
