...
def agent_environment(tmp_path, monkeypatch):
    ...
    for name in (
        ...
        monkeypatch.setattr(
            agent_read, name, partial(getattr(agent_read, name), index=index),
        )
    monkeypatch.setattr(
        session_search,
        "search_sessions",
...
def test_session_read_requires_engine_issued_reference(agent_environment):
    ...
    searched = agent_read.session_read("claude", ref=ref, terms=["支付"])
    assert searched["mode"] == "search"
    assert "matches" in searched

    with pytest.raises(AgentRequestError):
        agent_read.session_read("claude")
    with pytest.raises(AgentRequestError):
        agent_read.session_read("claude", ref="")
...
def test_context_is_turn_bounded_redacted_and_byte_bounded(agent_environment):
    ...
    with_output = agent_read.get_session_context(
...
def test_context_locator_is_required_for_agent_rewrite(agent_environment):
    ref = _claude_ref()
    context = agent_read.get_session_context(
...
def test_content_search_returns_editable_message_locators(agent_environment):
    ref = _claude_ref()
    result = agent_read.search_session_content(
    ...
    with pytest.raises(AgentRequestError):
        agent_read.search_session_content("claude", ref, [], limit=20)
