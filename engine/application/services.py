...
from ..adapters.codex.reader import session_id as codex_session_id
...
def resume_command(tool: str, sid: str, cwd: str) -> dict:
    return {"tool": tool, "session_id": sid, "cwd": cwd}
...
def list_models(tool: str) -> dict:
    ...
    models: list[dict] = []
    try:
        models, source, default = adapter(tool).model_provider()
    except Exception as e:
        error = str(e)[:400]
        if tool == "claude":
