...
def new_ordered_id(prefix: str, ordinal: int) -> str:
...
def _message_times(messages: list[Message], now: int) -> list[int]:
    ...
    for value in parsed:
        candidate = (
            ...
            else (previous + 1 if previous is not None else fallback)
        )
...
def _normalize_payload_message_times(payload: dict) -> None:
    ...
    for value in parsed:
        candidate = (
            value
            if value is not None
            else (previous_created + 1 if previous_created is not None else fallback)
        )
        created = (
            candidate
            if previous_created is None
            else max(candidate, previous_created + 1)
        )
    ...
    for message, source_time, original_created, created in zip(
        ...
        if "completed" in source_time:
            ...
            duration = (
                max(0, original_completed - original_created)
                if original_completed is not None and original_created is not None
                else 0
            )
...
def _task_part(
    ...
    part = clone(tpl["part.tool"])
    prompt = edge.prompt if edge else ""
    part.update(
        {
            "id": new_ordered_id("prt", ordinal),
            "messageID": mid,
            "sessionID": sid,
            "type": "tool",
            "tool": "task",
            "callID": (
                edge.source_call_id
                if edge and edge.source_call_id
                else source_call_id or "call-" + secrets.token_hex(8)
            ),
            "state": {
                "status": "completed",
                "input": {
                    "description": child.title or "migrated subagent",
                    "prompt": prompt,
...
def canonical_payload(
    ...
    info.update(
        {
            ...
            "time": {"created": session_created, "updated": session_updated},
        }
    )
    ...
    for m, message_time in zip(sess.messages, message_times):
        mid = new_id("msg")
        minfo = clone(tpl.get(f"msg.{m.role}", tpl["msg.user"]))
        minfo.update({"id": mid, "sessionID": sid})
        if m.role == "assistant":
            if last_user_mid is None:
                last_user_mid = new_id("msg")
                parent_info = clone(tpl["msg.user"])
                parent_info.update(
                    {
                        "id": last_user_mid,
                        "sessionID": sid,
                        "role": "user",
                        "time": {"created": message_time - 1},
                        "agent": "build",
                        "model": {"providerID": provider_id, "modelID": model_id},
                        "summary": {"diffs": []},
                    }
                )
                parent_part = clone(tpl["part.text"])
                parent_part.update(
                    {
                        "id": new_ordered_id("prt", 0),
                        "messageID": last_user_mid,
                        "sessionID": sid,
                        "type": "text",
            ...
            minfo["time"] = {"created": message_time, "completed": message_time}
            minfo["finish"] = "stop"
            minfo.update(
                {
                    "mode": "build",
        ...
        def add_part(ptype, fill):
            ...
            p.update(
                {
                    "id": new_ordered_id("prt", len(parts)),
                    "messageID": mid,
                    "sessionID": sid,
                }
            )
        ...
        def add_tool_part(tool, native_input, output, title, metadata, canonical_tool):
            st = clone(tpl["part.tool"]["state"])
            ...
            return add_part(
                "tool",
                {"tool": tool, "callID": "call-" + secrets.token_hex(8), "state": st},
            )
        ...
        for b in m.blocks:
            if b.kind == "text":
                ...
                if t.op == CanonicalOp.AGENT_SPAWN:
                    candidates = [
                        candidate
                        for candidate in sess.agent_edges
                    ...
                    edge = next(
                        (
                            ...
                            if t.source_call_id
                            and candidate.source_call_id == t.source_call_id
                        ),
                        None,
                    )
                    if edge is None:
                    ...
                    if (
                        ...
                        and (decision is None or decision.rendered is not None)
                    ):
                        parts.append(
                            _task_part(
                                tpl,
                    ...
                    continue
                writer = tool_calls.OP_WRITERS.get(t.op)
                if (
                    (decision is not None and decision.rendered is None)
                    or writer is None
                    or not has_valid_tool_input(t.op, t.input)
                    or not writer(add_tool_part, t)
                ):
                    params = {"tool_name": t.name}
                    if decision is not None:
                        params.update(
        ...
        for child_id, edge in edges.items():
            if (
                child_id in linked_children
                or edge.spawn_message_id != m.source_id
                or child_id not in children
                or child_id not in sid_map
            ):
                continue
            parts.append(
                _task_part(
...
def remap_payload(
    payload: dict, sid: str, cwd: str, parent_sid: str | None, sid_map: dict[str, str]
) -> dict:
    ...
    for message in payload.get("messages", []):
        ...
        if old_id:
            message_ids[old_id] = new_id("msg")

    last_user_mid = None
    for message in payload.get("messages", []):
        minfo = message["info"]
        old_id = minfo.get("id")
        mid = message_ids.get(old_id) or new_id("msg")
        minfo["id"] = mid
        minfo["sessionID"] = sid
        if minfo.get("role") == "user":
            last_user_mid = mid
        elif minfo.get("role") == "assistant":
            minfo["parentID"] = message_ids.get(minfo.get("parentID"), last_user_mid)
            if minfo["parentID"] is None:
                minfo.pop("parentID")

        for ordinal, part in enumerate(message.get("parts", [])):
            part["id"] = new_ordered_id("prt", ordinal)
            part["messageID"] = mid
            part["sessionID"] = sid
...
def ensure_task_links(
    payload: dict, sess: Session, sid: str, sid_map: dict[str, str], tpl: dict
) -> None:
    linked = set()
    for message in payload.get("messages", []):
    ...
    now = (
        max(
            (value for value in message_times if isinstance(value, int)),
    ...
    for child in sess.children:
        ...
        spawn_message = next(
            (
                ...
                for message in payload.get("messages", [])
                if edge
                and edge.spawn_message_id
                and message.get("info", {}).get("id") == edge.spawn_message_id
            ),
            None,
        )
        if spawn_message is not None:
            ...
            parts.append(
                _task_part(
                    tpl, sid, minfo["id"], len(parts), child, target_child, edge, when
                )
            )
            ...
            continue
        mid = new_id("msg")
        minfo = clone(tpl["msg.assistant"])
        cwd = payload["info"]["directory"]
        provider_id = str(sess.model_provider or "openai")
        model_id = str(sess.model or "gpt-5.6-sol")
        if last_user is None:
            last_user = new_id("msg")
            user_info = clone(tpl["msg.user"])
            user_info.update(
                {
                    "id": last_user,
                    "sessionID": sid,
