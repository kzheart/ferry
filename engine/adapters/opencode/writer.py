...
def _task_part(
    ...
    part.update(
        {
            "id": _new_ordered_id("prt", ordinal),
            "messageID": mid,
            "sessionID": sid,
            "type": "tool",
            "tool": "task",
...
def _canonical_payload(
    ...
    for m, message_time in zip(sess.messages, message_times):
        ...
        def add_part(ptype, fill):
            ...
            p.update(
                {
                    "id": _new_ordered_id("prt", len(parts)),
                    "messageID": mid,
                    "sessionID": sid,
                }
            )
...
def write(
    ...
    for node in sessions:
        ...
        payload = (
            _clone(explicit_payload) if isinstance(explicit_payload, dict) else None
        )
